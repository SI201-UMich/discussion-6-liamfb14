...
###############################################################################
    def load_results(self, table):
        ...
        '''
        race_dict = {}
        headers = table[0][1:]  # Get race names from first row, skipping "Horse"
        for row in table[1:]:   # Skip header row
            horse = row[0]
            race_dict[horse] = {}
            for i, race in enumerate(headers):
                race_dict[horse][race] = float(row[i + 1])
        return race_dict
...
###############################################################################
    ...
    def horse_fastest_race(self, horse):
        ...
        return (fastest_race, races[fastest_race])
...
###############################################################################
    ...
    def get_average_time(self):
        ...
        for horse, races in self.race_dict.items():
            ...
            averages[horse] = sum(times) / len(times)
        return averages
